    ...
    for property in response_dict['Results']:
        try:
            if property['Building']['Type'] == "House":
                ...
                M = (P * R * (1 + R) ** N) / (((1 + R) ** N) - 1)
                T = WINDSOR_PROPERTY_TAX * int(property['Property']['PriceUnformattedValue']) / 12
                beds = 0
                ...
                print(f"{property['Building']['StoriesTotal']} story {property['Building']['Type']} | {property['Building']['Bedrooms']} bed(s), {property['Building']['BathroomTotal']} bath(s)")
                ...
                print(f"Assuming average room price of {AVERAGE_ROOM_PRICE}, total revenue/month = ${AVERAGE_ROOM_PRICE * beds}")
                print(f"Profit = {AVERAGE_ROOM_PRICE * beds - total_cost:.2f}, down payment of ${int(property['Property']['PriceUnformattedValue']) * DOWN_PAYMENT:.2f}\n")
        except KeyError:
            print(f"=================================================================\nignoring {property['Property']['Address']['AddressText']}\n=================================================================")
